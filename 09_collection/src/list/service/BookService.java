...
public class BookService {
	 ...
	 * @return searchList
	 */
	public List<BookDTO> selectPrice(int MIN_NUM, int MAX_NUM) {
		
		List<BookDTO> searchList = new ArrayList<BookDTO>();
		
		for(BookDTO book : bookList) {
			if(book.getPrice() >= MIN_NUM && book.getPrice() <= MAX_NUM) {
				searchList.add(book);
			}
		}
		return searchList;
	 ...
	 * 전달 받은 index요소 제거하기
	 * @param index
	 * @return null : index 범위가 맞지 안흥ㅁ
	 */
	public BookDTO removeBook(int index) {
		if(index >= bookList.size() || index < 0) return null;
