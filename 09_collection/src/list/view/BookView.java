...
public class BookView {
	...
	private void selectIndex() {
		...
		// 전달 한 index번째 요소가 존재하면 BookDTO 객체 주소
		// 없다면 null 반환
...
//		if(searchList.size() == 0) {
	...
	private void selectPrice() {
		...
		System.out.print("검색할 책의 가격 범위 입력(최소값, 최대값) : ");
		int MIN_NUM = sc.nextInt();
		int MAX_NUM = sc.nextInt();
		
		List<BookDTO> searchList = service.selectPrice(MIN_NUM, MAX_NUM);
	...
	private void removeBook() {
		...
		BookDTO searchList = service.removeBook(index);
		
		if(searchList == null) {
			System.out.println("해당 인덱스에 존재하는 책이 없습니다");
			return;
		}
		System.out.println("[" + searchList.getTitle() + "]" + " 책이 목록에서 제거 되었습니다");
	...
	private void changePrice() {
		...
		// 책이 존재하면 가격 입력 받기
		System.out.print(book.getTitle() + "의 새로운 가격 입력 : ");
