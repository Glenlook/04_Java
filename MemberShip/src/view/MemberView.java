...
public class MemberView {
	...
	private void updateAmount() throws IOException {
		...
		String targerName = br.readLine();
		
		// 이름이 일치하는 회원 모두 조회
		List<Member> searchList = service.selectName(targerName);
		...
		if(searchList.size() > 1) {
			...
			System.out.println("선택할 회원의 번호 입력 : ");
	...
	private void updateMember() throws IOException {
		...
		String result = service.updatePhone(target, newPhone);
	...
	private void removeMember() throws IOException {
		...
		List<Member> searchList = service.selectName(targetName);
			...
			// 정말 탈퇴를 할 것인지 확인
			System.out.print("정말 탈퇴 처리하시겠습니까? (y/n) : ");
